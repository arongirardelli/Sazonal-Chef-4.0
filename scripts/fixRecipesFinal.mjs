...
import { recipeCorrectionsFinal } from './recipeCorrectionsFinal.mjs'
...
async function main() {
  console.log(`Corrigindo ${recipeCorrectionsFinal.length} receitas finais...`)
  console.log('URL:', url)
  console.log('Service Role Key:', serviceRoleKey ? '✅ Configurada' : '❌ Não configurada')
  
  for (const recipe of recipeCorrectionsFinal) {
    await updateRecipe(recipe)
    // Pequena pausa entre as atualizações
    await new Promise(resolve => setTimeout(resolve, 1000))
  }
  
  console.log('✅ Processo de correção final concluído!')
  console.log('🎉 Todas as receitas foram corrigidas com sucesso!')
}
