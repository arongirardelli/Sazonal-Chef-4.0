...
import { recipeCorrectionsBatch2 } from './recipeCorrectionsBatch2.mjs'
...
async function main() {
  console.log(`Corrigindo ${recipeCorrectionsBatch2.length} receitas do segundo lote...`)
  console.log('URL:', url)
  console.log('Service Role Key:', serviceRoleKey ? '✅ Configurada' : '❌ Não configurada')
  
  for (const recipe of recipeCorrectionsBatch2) {
    await updateRecipe(recipe)
    // Pequena pausa entre as atualizações
    await new Promise(resolve => setTimeout(resolve, 1000))
  }
  
  console.log('✅ Processo de correção do segundo lote concluído!')
}
